...
// five tabs: the four content feeds and Settings — the reader's own lists
// (saved, continue reading) and the subsites live in the header drawer now
const TABS: Tab[] = [
  {
    label: "Home",
    href: "/",
  ...
  {
    // opens a chooser like Read does — the two feeds are different enough
    // (vertical reels vs. full episodes) that landing on one surprised readers
    label: "Watch",
    href: "/reels",
    match: ["/watch-videos"],
    children: [
...
export default function MobileTabBar() {
  ...
  const panelLinks: PanelLink[] = TABS.find((t) => t.label === openTab)?.children ?? [];
  ...
  return (
    ...
    >
      {/* tap-anywhere-else dismissal; it stops at the bar so the bar itself stays
          un-dimmed. Portalled to the body because the bar carries a transform
      ...
      {/* drop-up panel, sits on top of the bar */}
      {panelOpen && (
