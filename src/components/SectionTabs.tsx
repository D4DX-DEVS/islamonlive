...
export default function SectionTabs({
  ...
  return (
    <div className="mb-4 flex flex-wrap items-center gap-x-4 gap-y-2 border-l-4 border-purple-800 pl-3">
      <h2 className="text-xl font-extrabold text-zinc-900">{title}</h2>
      {/* pushed right of the title, like the live site's section menus */}
      <div className="-mx-1 flex flex-1 flex-wrap items-center justify-center gap-1 overflow-x-auto px-1 sm:gap-2">
        {labels.map((label, n) => (
          <button
            key={label}
            type="button"
