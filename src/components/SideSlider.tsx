    ...
    <div className={`group relative overflow-hidden rounded-xl bg-zinc-900 ${className}`}>
      <Link href={s.href} className="relative block h-full">
        ...
        <div className="absolute inset-x-0 bottom-0 p-4">
          {s.category && (
            <span className="mb-1.5 inline-block rounded bg-purple-700 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white">{s.category}</span>
          )}
          <h3 className="line-clamp-2 text-sm font-bold leading-snug text-white lg:text-base" dangerouslySetInnerHTML={{ __html: s.title }} />
